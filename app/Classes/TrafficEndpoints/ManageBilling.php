...
class ManageBilling
{
    private string $_trafficEndpointId;
    ...
    private $collections = [
        'billing_entities' => 'endpoint_billing_entities',
        'billing_payment_methods' => 'endpoint_billing_payment_methods',
        'billing_payment_requests' => 'endpoint_billing_payment_requests',
        'billing_chargebacks' => 'endpoint_billing_chargebacks',
        'billing_adjustments' => 'endpoint_billing_adjustments',
    ];

    public function __construct(string $trafficEndpointId, array $payload = [])
    {
        $this->_trafficEndpointId = $trafficEndpointId;
    ...
    {
        return TrafficEndpoint::query()->find($this->_trafficEndpointId)->toArray();

        // $token = $this->_trafficEndpointId;
    ...
    {
        // $where = ['endpoint' => $this->_trafficEndpointId];
        // $mongo = new MongoDBObjects($this->collections['billing_payment_methods'], $where);
        // $array = $mongo->findMany();

        $array = TrafficEndpointBillingPaymentMethods::query()->where('endpoint', '=', $this->_trafficEndpointId)->get()->toArray();
    ...
    public function billing_payment_method_files($id): array
    {

        // TODO: Aff later
        // if (!permissionsManagement::is_current_user_role('financial')) {
        ...
        $data = TrafficEndpointBillingPaymentMethods::query()->find($id);
        if ($data == null) {
            return [];
        }
    ...
    {
        $array = TrafficEndpointBillingEntities::query()->where('endpoint', '=', $this->_trafficEndpointId)->get()->toArray();

        // $partner = $this->_trafficEndpointId;
        ...
        //$storage = new FileStorage('billing_entity', $ticket_number);
        // $storage = new CDN('billing_entity');
    ...
    public function get_billing_entities()
    {
        ...
        $data = TrafficEndpointBillingEntities::query()->where('endpoint', '=', $this->_trafficEndpointId)->get()->toArray();
    ...
    {
        $data = TrafficEndpointBillingEntities::query()->find($id)->toArray();
        // $where = ['_id' => new \MongoDB\BSON\ObjectId($id)];
        // $mongo = new MongoDBObjects($this->collections['billing_entities'], $where);
        // $data = $mongo->find();
        if (isset($data['files']) && count($data['files']) > 0) {
            // $f = new CDN('billing_entity');
            $storage = StorageWrapper::instance('billing_entity');
            // try {
            //     $storage->login();
            // } catch (\Exception $ex) {
    ...
    {
        $query = TrafficEndpointBillingEntities::query()->find($id);
        $item = $query->toArray();
        // $where = ['_id' => new \MongoDB\BSON\ObjectId($id)];
        // $mongo = new MongoDBObjects($this->collections['billing_entities'], $where);
    ...
    {
        $model = TrafficEndpointBillingEntities::findOrFail($id);
    ...
    public function add_billing_entity()
    {
        ...
        $model = new TrafficEndpointBillingEntities();
        $model->fill($insert);
        $model->save();
        $id = $model->id;

        // $mongo = new MongoDBObjects($this->collections['billing_entities'], $insert);
    ...
    public function feed_billing_payment_methods(): array
    {
        ...
        // $storage = new CDN('billing_payment_methods');
        ...
        foreach ($array as $billing) {
            ...
            // $files = [];
            // if (isset($billing['files']) && count($billing['files']) > 0) {
            //     $files = StorageWrapper::get_files($billing['files']); // $storage->get_files($billing['files']);
            // }
    ...
    public function set_default_active_billing_payment_methods()
    {
        $partner = $this->_trafficEndpointId;

        // $data = TrafficEndpointBillingPaymentMethods::query()->where('endpoint', '=', $this->_trafficEndpointId)->whereIn('status', ['1', 1])->first();
        // print_r($data);
        // return false;
        $where = ['endpoint' => $partner, 'status' => ['$in' => ['1', 1]]];
        $mongo = new MongoDBObjects($this->collections['billing_payment_methods'], $where);
        if ($mongo->count() > 0) {
            return;
        }
        ...
        // $query = TrafficEndpointBillingPaymentMethods::query()->where('endpoint', '=', $this->_trafficEndpointId)->first();
        // $query->update($update);

        $where = ['endpoint' => $partner];
        $mongo = new MongoDBObjects($this->collections['billing_payment_methods'], $where);
        $data = $mongo->find();
    ...
    public function active_billing_payment_methods($id)
    {
        ...
        $query = TrafficEndpointBillingPaymentMethods::query()->where('endpoint', '=', $this->_trafficEndpointId);
        $query->update(['status' => 0]);

        $query = TrafficEndpointBillingPaymentMethods::query()->find($id);
        ...
        // $mongo = new MongoDBObjects($this->collections['billing_payment_methods'], $where);
        // $update = [];
        // $update['status'] = 1; //$this->request_post('status');
        // $mongo->update($update);
    ...
    {
        $data = TrafficEndpointBillingPaymentMethods::query()->find($id)->toArray();
    ...
    {
        $query = TrafficEndpointBillingPaymentMethods::query()->where('endpoint', '=', $this->_trafficEndpointId)->whereIn('status', ['1', 1])->first('_id');
        return $query->id;
        ...
        // return MongoDBObjects::get_id($id);
    }
    ...
    {
        $model = TrafficEndpointBillingPaymentMethods::findOrFail($id);
        ...
        $update['currency_crypto_wallet_type'] = $this->request_post('currency_crypto_wallet_type');
    ...
    public function add_billing_payment_methods(): string
    {
        ...
        $insert['currency_crypto_wallet_type'] = $this->request_post('currency_crypto_wallet_type');
    ...
    public function feed_billing_chargebacks(): array
    {
        ...
        $array = TrafficEndpointBillingChargebacks::query()->where('endpoint', '=', $this->_trafficEndpointId)->get()->toArray();
        ...
        foreach ($array as $billing) {
            ...
            if (isset($billing['screenshots']) && count($billing['screenshots']) > 0) {
                ...
                //     $files_html .= (empty($files_html) ? '' : ', ') . '<a href="' . $file['link'] . '"  class="file-preview"  data-filename="' . $file['original_file_name'] . '">' . $file['original_file_name'] . '</a>';
                // }
            }
    ...
    public function get_billing_chargebacks_balans()
    {
        ...
        // $array = TrafficEndpointBillingChargebacks::
        // DB::connection('mongodb')->table('endpoint_billing_chargebacks')->raw()
        // $data = DB::collection('endpoint_billing_chargebacks')->raw(function ($collection) {
    ...
    {
        $data = TrafficEndpointBillingChargebacks::query()->find($id)->toArray();
        // $where = ['_id' => new \MongoDB\BSON\ObjectId($id)];
        // $mongo = new MongoDBObjects($this->collections['billing_chargebacks'], $where);
        // $data = $mongo->find();
        ...
        if (isset($data['screenshots']) && count($data['screenshots']) > 0) {
            ...
            // } catch (\Exception $ex) {
            // }
            $data['screenshots'] = StorageWrapper::get_files($data['screenshots']); // $storage->get_files($data['screenshots']);
        }
        unset($data['_id']);
        return $data;
    }
    ...
    {
        // $google_auth_key = $this->request_post('google_auth_key');
        ...
        $query = TrafficEndpointBillingChargebacks::query()->find($id);
        ...
        if (!empty($item['payment_request'])) {
            $query = TrafficEndpointBillingPaymentRequests::query()->find($item['payment_request']);
            // $where = ['_id' => new \MongoDB\BSON\ObjectId($item['payment_request'])];
    ...
    public function update_billing_chargebacks($id)
    {
        ...
        $model = TrafficEndpointBillingChargebacks::findOrFail($id);
        ...
        if (!empty($model->payment_request)) {
            $query = TrafficEndpointBillingPaymentRequests::query()->find($model->payment_request);
            $query->update(['chargeback' => false]);
            // $where = ['_id' => new \MongoDB\BSON\ObjectId($item['payment_request'])];
            // $mongo = new MongoDBObjects($this->collections['billing_payment_requests'], $where);
            // $mongo->update(['chargeback' => false]);
        }

        if (!empty($update['payment_request'])) {
            $query = TrafficEndpointBillingPaymentRequests::query()->find($update['payment_request']);
            $query->update(['chargeback' => true]);

            // $where = ['_id' => new \MongoDB\BSON\ObjectId($update['payment_request'])];
            // $mongo = new MongoDBObjects($this->collections['billing_payment_requests'], $where);
            // $mongo->update(['chargeback' => true]);
        }
    ...
    public function add_billing_chargebacks()
    {
        ...
        // $mongo = new MongoDBObjects($this->collections['billing_chargebacks'], $insert);
        // $id = $mongo->insertWithToken();

        if (!empty($insert['payment_request'])) {
            $query = TrafficEndpointBillingPaymentRequests::query()->find($insert['payment_request']);
            $query->update(['chargeback' => true]);

            // $where = ['_id' => new \MongoDB\BSON\ObjectId($insert['payment_request'])];
            // $mongo = new MongoDBObjects($this->collections['billing_payment_requests'], $where);
            // $update = ['chargeback' => true];
            // $mongo->update($update);
        }
    ...
    {
        $array = TrafficEndpointBillingAdjustments::query()->where('endpoint', '=', $this->_trafficEndpointId)->get()->toArray();
    ...
    public function get_billing_adjustments_balans()
    {
        ...
        // $result = DB::collection('billing_adjustments')->raw(function ($collection) {
        //     return $collection->aggregate(array(
        //         [
        //             'group' => [
        //                 'count' => [
    ...
    {
        $data = TrafficEndpointBillingAdjustments::query()->find($id)->toArray();
    ...
    public function remove_billing_adjustments($id)
    {
        ...
        $query = TrafficEndpointBillingAdjustments::query()->find($id);
        return $query->delete();
    ...
    public function update_billing_adjustments($id)
    {
        ...
        $model = TrafficEndpointBillingAdjustments::findOrFail($id);
        // $where = ['_id' => new \MongoDB\BSON\ObjectId($id)];
        // $mongo = new MongoDBObjects($this->collections['billing_adjustments'], $where);
    ...
    public function add_billing_adjustments()
    {
        ...
        $model = new TrafficEndpointBillingAdjustments();
        $model->fill($insert);
        $model->save();
        $id = $model->id;
    ...
    public function billing_payment_requests_group_statuses($time_range)
    {
        ...
        $where = [
            '$and' => [
                ['TrafficEndpoint' => (string)$partner],
                [
                    '$or' => [
                        ['Timestamp' => ['$gte' => $time_range['start'], '$lte' => $time_range['end']]],
                        ['endpointDepositTimestamp' => ['$gte' => $time_range['start'], '$lte' => $time_range['end']]],
                    ]
                ],
                [
                    '$or' => [
                        ['last_try_crg_percentage_id' => ['$exists' => true, '$ne' => null]],
                        ['last_try_broker_crg_percentage_id' => ['$exists' => true, '$ne' => null]]
                    ]
                ],
                [
                    '$or' => [
                        ['crg_status' => ['$exists' => true, '$ne' => null]],
                        ['broker_crg_status' => ['$exists' => true, '$ne' => null]]
                    ]
                ]
            ]
        ];
    ...
    public function billing_payment_requests_buildParameterArray()
    {
        ...
        /*$_deposit_revenue = array('deposit_revenue' =>  [
            ...
            'formula' => '
                    ...
                    if ( __(bool)match_with_broker__ == FALSE ) {
                        ...
                        if ( __(bool)broker_crg_ftd_uncount__ == FALSE && __(bool)depositor__ == TRUE && __(bool)deposit_disapproved__ == FALSE && __endpointDepositTimestamp__ >= ' . $this->start . ' && __endpointDepositTimestamp__ <= ' . $this->end . ' ) {
        ...
        foreach ($metrics as $metrics) {
            ...
            if ($metrics == 'crg_revenue') {
                $array['crg_revenue'] = array('crg_revenue' => [
                    'type' => 'sum',
                    'field_type' => 'float',
                    'formula' => '
                        if ( __(bool)crg_deal__ == TRUE && __Timestamp__ >= ' . $this->start . ' && __Timestamp__ <= ' . $this->end . ' ) {
                            return __crg_revenue__;
                        }
                        return 0;
                    ',
                    'formula_return' => false
                ...
                $array['Depositors'] = array('depositor' => [
                    ...
                    'formula' => '
                                if (
                                    ((__(bool)depositor__ == TRUE && __(bool)deposit_disapproved__ == FALSE) || __(bool)fakeDepositor__ == TRUE )
                                    && __endpointDepositTimestamp__ >= ' . $this->start . ' && __endpointDepositTimestamp__ <= ' . $this->end . ' ) {
                                        if ( __(bool)crg_deal__ == TRUE && __(bool)crg_already_paid__ == TRUE) {
                                            return false;
                                        }else{
                                            return true;
                                        }
    ...
    private function _get_feed_billing_payment_request($timeframe)
    {
        ...
        $conditions = [
            'TrafficEndpoint' => [$this->_trafficEndpointId],
            'match_with_broker' => 1,
            'test_lead' => 0
        ];
        ...
        $time = [
            'where' => [
                '$or' => [
                    ...
                    ['endpointDepositTimestamp' => array('$gte' => $time_range['start'], '$lte' => $time_range['end'])]
                ]
            ]
        ];
    ...
    private function _get_feed_group_by_billing_payment_request($data)
    {
        ...
        $find = TrafficEndpoint::all()->toArray();
        // $mongo = new MongoDBObjects('TrafficEndpoints', []);
        // $find = $mongo->findMany();
        $TrafficEndpoints = array();

        foreach ($find as $supply) {
            $id = $supply['_id'];
            $TrafficEndpoints[$id] = ($supply['token'] ?? '');
        }
        ...
        foreach ($data as $ndata => $datas) {
            ...
            // $name = 'crg_deal[' . ($datas['crg_deal'] ? '1' : '0') . '] | isCPL[' . ($datas['isCPL'] ? '1' : '0') . ']';
            // if (($datas['crg_deal'] ?? false) == false) {
        ...
        foreach ($groupBy as $groupKey => $values) {
            foreach ($values as $datas) {
                ...
                $total['ftds'] ??= 0;
                $total['ftds'] += $datas['Depositors'] ?? 0;

                $total['leads'] ??= 0;
                $total['leads'] += $datas['Leads'] ?? 0;
                // echo $groupKey;
                // print_r($datas);
                // CRG
                if (strpos($groupKey, 'crg_deal[0]') !== false) { // Invalid leads
                    $total['invalid'] ??= 0;
                    $total['invalid'] += 1;
                } else if (strpos($groupKey, 'crg_deal[1]') !== false) { // Valid leads

                    //$name = $groupKey . '///crg_percentage_id[' . $datas['crg_percentage_id'] . '] | crg_percentage[' . $datas['crg_percentage'] . '] | crg_payout[' . $datas['crg_payout'] . '] | cost[' . $datas['cost'] . ']';//| cr[' . $datas['cr'] . ']

                    /*$name = $groupKey . '///' .
                        'crg_percentage_id[' . (!empty($datas['changed_crg_percentage_id'] ?? '') ? $datas['changed_crg_percentage_id'] : $datas['crg_percentage_id']) . '] | ' .
                        'crg_percentage[' . $datas['crg_percentage'] . '] | ' .
                        'crg_payout[' . $datas['crg_payout'] . '] | ' .
                        'cost[' . $datas['cost'] . ']'; */

                    $crg_percentage_id = (!empty($datas['changed_crg_percentage_id'] ?? '') ? $datas['changed_crg_percentage_id'] : ($datas['crg_percentage_id'] ?? ''));

                    $crg_percentage = (!empty($datas['changed_crg_percentage'] ?? '') ? $datas['changed_crg_percentage'] : $datas['crg_percentage']);

                    $crg_payout = (!empty($datas['changed_crg_payout'] ?? '') ? $datas['changed_crg_payout'] : $datas['crg_payout']);

                    $name = $groupKey . '///crg_percentage_id[' . $crg_percentage_id . '] | crg_percentage[' . $crg_percentage . '] | crg_payout[' . $crg_payout . '] | cost[' . $datas['cost'] . ']';

                    if (!isset($subGroupBy[$name])) {
                        //$datas['items_by_row'] = 1;
                        $datas['total'] = $datas['cost'];
                        $datas['Leads'] = $datas['Leads'] ?? 0;
                    ...
                    // $id = $datas['_id'];
                    // if ($id == '619661bcf0cce472ec5487f4' || $id == '619c496572864271f8635d62' || $id == '61a11c2dc7583c4dcc38bdb4') {
                    //     echo PHP_EOL . '-----' . PHP_EOL . $name . PHP_EOL;
                    //     echo $id . ' | ('.$subGroupBy[$name]['Depositors'].')';
                    // }
                    ...
                    // CPA
                        ...
                        if (!isset($subGroupBy[$name])) {
                            ...
                            $subGroupBy[$name]['Depositors'] += $datas['Depositors'] ?? 0;
                            $subGroupBy[$name]['total'] = $subGroupBy[$name]['total'] +  $datas['cost'];
                            $subGroupBy[$name]['crg_revenue'] = $subGroupBy[$name]['crg_revenue'] +  $datas['crg_revenue'];
                            // $subGroupBy[$name]['crg_payout'] = $subGroupBy[$name]['crg_payout'] +  $datas['crg_payout'];
                        }
                    }

                if (!empty($datas['_id']) && $datas['Depositors'] > 0) {
    ...
    public function get_billing_payment_request($id)
    {
        ...
        // $query = TrafficEndpointBillingPaymentRequests::query()->find($id);
        // if (!isset($data)) {
        //     return [];
        // }
        // $data = $query->toArray();
    ...
    {
        $data = TrafficEndpointBillingPaymentRequests::query()->find($id)->toArray();
        // $where = ['_id' => new \MongoDB\BSON\ObjectId($id)];
        // $mongo = new MongoDBObjects($this->collections['billing_payment_requests'], $where);
        // $data = $mongo->find();
        $leads = json_decode($data['json_leads'], true);
    ...
    public function feed_billing_payment_request($payment_request = [], $data = null)
    {
        ...
        // $html .= '<div class="pb-1"><a href="#" class="small" onclick="download_crg_full_deduction_report(\'' . $timeframe . '\')">Download All Deduced Leads</a></div>';

        // $html .= '<table class="table">';
        // $html .= '<tr style="background-color: #292f4c;color:white;">' .
        //     '<th>#</th>' .
        //     '<th>Nomination</th>' .
        //     '<th>Count</th>' .
        //     '<th>Cost</th>' .
        //     '<th>Total</th>' .
        //     '</tr>';
        foreach ($subGroupBy['items'] as $groupKey => $datas) {

            if (($datas['cost'] ?? 0) == 0) {
                continue;
            }

            // $html .= '<div style="font-weight: normal; color: gray;">' . $groupKey . '<pre>' . print_r($datas, true) . '</pre></div>';
            $datas['cr'] = round($datas['Leads'] > 0 ? (($datas['Depositors'] / $datas['Leads']) * 100) : 0, 2);
            $ftds = $datas['Depositors'];
            $invalidCount = strpos($groupKey, 'crg_deal[0]') !== false ? $datas['Leads'] : 0;
            if (strpos($groupKey, 'crg_deal[1]') !== false) {

                $crg_percentage_id = null;
                if (isset($datas['changed_crg_percentage_id'])) {
                    $crg_percentage_id = $datas['changed_crg_percentage_id'];
                }
                if (empty($crg_percentage_id) && isset($datas['crg_percentage_id'])) {
                    $crg_percentage_id = $datas['crg_percentage_id'];
                }
                if (empty($crg_percentage_id) && isset($datas['last_try_crg_percentage_id'])) {
                    $crg_percentage_id = $datas['last_try_crg_percentage_id'];
                }
                ...
                $cnt = 0;
                if ($datas['Depositors'] > $datas['Leads'] && $datas['cost'] > 299) {
                    $cnt = number_format($datas['Depositors'], 0, '.', ' ') . ' FTD\'s';
                } elseif ($crg_percentage > $datas['cr'] || $datas['cost'] < 299) { //для случая когда сделку разбило пополам по пейуту
                ...
                // $counties_str = $countries[$datas['country']];
                ...
                $active = (bool) BrokerPrivateDeal::where('_id', $crg_percentage_id)->first()?->max_crg_invalid ?? false;

                if ($active) {
                    $deducted = ($cnt ?? 0) > 0 ? (($invalidCount ?? 0) * 100) / ($cnt ?? 0) : 0;
                    $status .= ' - Max Deducted Leads ' . $deducted . '%';
                }

                $result['items'][] = [
                    '_id' => $crg_percentage_id,
                    'data' => $datas,
                    'count' => $cnt,
                    'invalid_count' => $invalidCount,
                    'ftd_count' => number_format($ftds, 0, '.', ' ') . ' FTD\'s',
                    'cost' => $datas['cost'],
                    'total' => $datas['total'],
                    'nomination' => '<strong>CRG. </strong>' . /*$datas['crg_leads'] . ' leads ' .*/ $counties_str .
                        ' with (min CRG ' . $crg_percentage . '%) cr ' . $datas['cr'] . '% ' . $status .
                        (
                            (Gate::has('custom[show_crg_deal_id]') && Gate::allows('custom[show_crg_deal_id]'))
                            ?
                            '<small style="display:block;color:gray;font-size:10px;">CRG ID: ' . $crg_percentage_id . '</small>'
                            :
                            ''
                        ) .
                        '<div style="display:none"><br/>(calculated by $' . $datas['crg_payout'] . ' payout, $' . $datas['crg_revenue'] . ' crg revenue)</div>
                        <div style="color:gray;display:none">' . print_r($datas, true) . '</div>'
                    // '<a href="#" class="small" onclick="load_crg_deal_details(\'' . $crg_percentage_id . '\', \'' . $timeframe . '\', $(this).next())">details</a>'
                ];

                // if CRG deal is fail
            } else if (strpos($groupKey, 'isCPL[1]') !== false) {
                //120 Leads Germany CPL 25 = $3000
                // $html .= $datas['Leads'] . ' leads ' . $countries[$datas['country']] . ' CPL ' . $datas['cost'] . ' = $' . $datas['cost'];
                ...
                $result['items'][] = [
                    ...
                    'cost' => $datas['cost'],
                    'total' => $datas['total'],
                    'nomination' => '<strong>CPL. </strong>' . $countries[strtolower($datas['country'])] . ' cr ' . $datas['cr'] . '%'
                ];
            } else {
                //3 FTD IT CPA 985 = $2955
                // $html .= $datas['Leads'] . ' FTD ' . $countries[strtolower($datas['country'])] . ' CPA ' . $datas['cost'] . ' = $' . $datas['cost'];

                $result['items'][] = [
                    'data' => $datas,
                    'depositors' => number_format($datas['Depositors'], 0, '.', ' ') . ' FTD\'s',
                    'count' => number_format($datas['Depositors'], 0, '.', ' ') . ' FTD\'s',
                    'invalid_count' => $invalidCount,
                    'ftd_count' => number_format($ftds, 0, '.', ' ') . ' FTD\'s',
                    'cost' => $datas['cost'],
                    'total' => $datas['total'],
                    'nomination' => '<strong>CPA.</strong> ' . $countries[strtolower($datas['country'])] . ' cr ' . $datas['cr'] . '%' .
                        '<div style="color:gray;display:none">' . print_r($datas, true) . '</div>'
        ...
        // $html .=
        //     '<tr style="background-color: #292f4c;color:white;">' .
        //     '<td colspan="4">Total</td>' .
        //     '<td style="display:none">' . number_format($leads, 0, '.', ' ') . '</td>' .
        //     // '<td>$' . number_format($costs, 0, '.', ' ') . '</td>' .
    ...
    public function feed_billing_payment_requests()
    {
        ...
        $where = [
            'endpoint' => $this->_trafficEndpointId,
        ...
        $array = TrafficEndpointBillingPaymentRequests::query()
            ->where('endpoint', '=', $this->_trafficEndpointId)
            ->get()
        ...
        foreach ($array as $data) {
            ...
            // files
            $fields_files = ['affiliate_invoices', 'master_approve_files', 'final_approve_files', 'proof_screenshots'];
            $is_files = false;
    ...
    {
        $model = TrafficEndpointBillingPaymentRequests::query()->find($id);
    ...
    public function billing_payment_requests_master_approve($id): bool
    {
        ...
        $model = TrafficEndpointBillingPaymentRequests::findOrFail($id);
        ...
        $update = [
            ...
            'master_status' => 1,
            'master_approve_note' => empty($this->request_post('note', '')) ? $this->request_post('description', '') : $this->request_post('note', ''),
            // 'master_approve' => true,
        ];
    ...
    public function billing_payment_requests_final_approve($id): bool
    {
        ...
        $payload = [
            ...
            'hash_url' => $this->request_post('hash_url'),
        ];
        ...
        $message = $subject . ' :';
        $message .= ' Endpoint ' . ($endpoint['token'] ?? '');
    ...
    public function billing_payment_requests_real_income($id): bool
    {
        ...
        $payment_request = TrafficEndpointBillingPaymentRequests::query()->find($id)->toArray();
        // $where = ['_id' => new \MongoDB\BSON\ObjectId($id)];
        // $mongo = new MongoDBObjects($this->collections['billing_payment_requests'], $where);
        // $payment_request = $mongo->find();
        ...
        if ($delta != 0) {
            ...
            $insert = array();
            $insert['endpoint'] = $this->_trafficEndpointId;
    ...
    public function send_email_billing_payment_requests(string $payment_request_id, array $leads)
    {
        ...
        $endpoint = TrafficEndpoint::findOrFail($model->endpoint);
        ...
        if (!empty($subscribers)) {
            ...
            $balance = $this->feed_billing_general_balances();
            ...
            try {
                if (!empty($model->payment_method)) {
                    $payment_method = TrafficEndpointBillingPaymentMethods::findOrFail($model->payment_method);
    ...
    public function save_billing_payment_requests()
    {
        ...
        // $traffic_endpoint = TrafficEndpoint::findOrFail($this->_trafficEndpointId);
        // if (isset($traffic_endpoint) && $payment_request_type == 'prepayment' && (int)($traffic_endpoint->probation ?? 0) == 0) {
        //     $traffic_endpoint->update(['probation' => 2]);
        // }
        ...
        $model = new TrafficEndpointBillingPaymentRequests();
    ...
    public function billing_payment_requests_is_approve()
    {
        ...
        $query = TrafficEndpointBillingPaymentRequests::query()
            ->where('endpoint', '=', $this->_trafficEndpointId)
        ...
        // $data = new MongoDBObjects($this->collections['billing_payment_requests'], $where);

        // $query_data = $data->find();
    ...
    public function billing_payment_requests_files($id): array
    {
        ...
        $data = TrafficEndpointBillingPaymentRequests::query()->find($id);
        if ($data == null) {
            return [];
        }
        $billing = $data->toArray();
        ...
        $collection_files = [
            ...
            'proof_screenshots' => 'Proof Screenshots'
        ];
        ...
        foreach ($collection_files as $field_name => $title) {
            ...
            if (isset($billing[$field_name]) && count($billing[$field_name]) > 0) {
                $files = $storage->get_files($billing[$field_name]);
                // foreach ($files as $file) {
                // $files_html .= (empty($files_html) ? '' : ', ') . '<a href="' . $file['link'] . '" target="_blank" class="file-preview" data-filename="' . $file['original_file_name'] . '">' . $file['original_file_name'] . '</a>';
                // }
            }
            ...
            if ($metrics == 'cr') {
                ...
                if (!isset($array['Depositors'])) {
                    $array['Depositors'] = array('depositor' => [
                        'type' => 'count',
                        'formula' => '
                            if ( __(bool)depositor__ == TRUE && __(bool)deposit_disapproved__ == FALSE && __endpointDepositTimestamp__ >= ' . $this->start . ' && __endpointDepositTimestamp__ <= ' . $this->end . ' ) {
                                return true;
                            }
                            return false;
                        ',
    ...
    public function update_billing_balances_log(string $logId): bool
    {
        ...
        if ($mongo->update($this->_payload)) {
            $this->_payload['endpoint'] = $this->_trafficEndpointId;
            ...
            HistoryDB::add(HistoryDBAction::Update, 'billings_log', $this->_payload, $logId, 'endpoint', [], null, $description);
            return true;
        }
        return false;
    }
    ...
    public function feed_billing_general_balances(): array
    {
        ...
        // payment request
        $list = TrafficEndpointBillingPaymentRequests::query()->where('endpoint', '=', $this->_trafficEndpointId)->where('final_status', '=', 1)->get()->toArray();

        // $where = ['final_status' => 1, 'endpoint' => $this->_trafficEndpointId];
        // $mongo = new MongoDBObjects($this->collections['billing_payment_requests'], $where);
        // $list = $mongo->findMany();
        ...
        // echo '('.$payment_request_total['prepayment'].' + '.$payment_request_total['payment'].' - '.$cost.' + '.$chargebacks_balans.' + '.$adjustment_amounts.')';
    ...
    public function get_history_billing_general_balances($collection = '', $limit = 20)
    {
        ...
        $collections = [
            'endpoint_billing_entities',
            'endpoint_billing_chargebacks',
            'endpoint_billing_adjustments',
            'endpoint_billing_payment_methods',
            'endpoint_billing_payment_requests',
            'billings_log'
        ];
        ...
        if (empty($collection)) {
            $where['$and'][] = [
                '$or' => [
                    ...
                    [
                        '$and' => [
                            ['collection' => 'endpoint_billing_payment_methods'],
                            ['diff.status' => 1]
                        ]
                    ]
                ]
            ];
        } elseif ($collection == 'endpoint_billing_payment_methods') {
    ...
    public function get_array_history_billing_general_balances(): array
    {
        ...
        $collections = [
            'endpoint_billing_entities' => 'Billing Entities',
            'endpoint_billing_payment_methods' => 'Payment Methods',
            'endpoint_billing_payment_requests' => 'Payment Requests',
            'endpoint_billing_adjustments' => 'Adjustments',
            'endpoint_billing_chargebacks' => 'Chargebacks',
        ...
        if (isset($history_logs) && count($history_logs) > 0) {
            ...
            // return '<pre>' . print_r($payment_methods, true) . "</pre>";
            ...
            foreach ($history_logs as $history) {
                ...
                switch ($history['collection']) {
                    ...
                    case 'endpoint_billing_entities': {
                    ...
                    case 'endpoint_billing_chargebacks': {
                            // if (isset($diff['type'])) {
                            //     $data .= 'Type: ' . $diff['type'];
                            // }
                    ...
                    case 'endpoint_billing_adjustments': {
                            if (isset($diff['amount'])) {
                    ...
                    case 'endpoint_billing_payment_methods': {
                    ...
                    case 'endpoint_billing_payment_requests': {
    ...
    {
        $data = $this->_get_feed_crg_deal_details($timeframe);
        $details = [];

        //die(json_encode($data, JSON_PRETTY_PRINT));

        foreach ($data as $datas) {
            $crg_id = $datas['crg_percentage_id'] ?: $datas['last_try_crg_percentage_id'];
            if ($crg_id != $percentage_id) {
                continue;
            }

            // Build group details
            $details['Leads'] = $datas['Leads'] + ($details['Leads'] ?? 0);
            $details['CRG Leads'] = $datas['crg_leads'] + ($details['CRG Leads'] ?? 0);

            if (strtolower($datas['status']) == 'wrong number') {
                $details['Wrong Number'] = $datas['Leads'] + ($details['Wrong Number'] ?? 0);
            }
            if (strtolower($datas['status']) == 'language barrier') {
                $details['Language Barrier'] = $datas['Leads'] + ($details['Language Barrier'] ?? 0);
            }
            if (strtolower($datas['status']) == 'junk lead') {
                $details['Junk Lead'] = $datas['Leads'] + ($details['Junk Lead'] ?? 0);
            }
            if (!$datas['hit_the_redirect']) {
                $details['No Autologin'] = $datas['Leads'] + ($details['No Autologin'] ?? 0);
            }
            if (!empty($datas['crg_status'])) {
                $details[$datas['crg_status']] = $datas['Leads'] + ($details[$datas['crg_status']] ?? 0);
            }
            if ($datas['crg_deal'] == false && (!empty($datas['crg_percentage_id']) || !empty($datas['last_try_crg_percentage_id']))) {
                $details['Deduced Leads'] = $datas['Leads'] + ($details['Deduced Leads'] ?? 0);
            }
        }

        // $html = '<table class="table-sm small">';

        $tmp = $details['Deduced Leads'] ?? 0;
        unset($details['Deduced Leads']);
        $details['Deduced Leads'] = $tmp;
        ...
        // if ($details['Deduced Leads'] > 0) {
        //     $html .= '<a href="#" class="small" onclick="download_crg_deduction_report(\'' . $percentage_id . '\', \'' . $timeframe . '\')">Download deduction report</a>';
        // }
    ...
    function _export_crg_deduction_report($percentage_id, $timeframe)
    {
        ...
        $filename = 'export_crg_deduction_report_' . date('Y-m-d') . '.csv';

        // use keys as column titles
        $header = ['email', 'status', 'crg_status'];

        foreach ($data as $key => $datas) {
            $crg_id = $datas['crg_percentage_id'] ?: $datas['last_try_crg_percentage_id'];
            if ($datas['crg_deal'] == true || $crg_id != $percentage_id) {
                unset($data[$key]);
            }

            // --- Decrypt --- //
            CryptHelper::decrypt_lead_data_array($datas);
        }
        $this->_export_csv($filename, $header, $data);
    }
    ...
    function _export_crg_full_deduction_report($timeframe)
    {
        ...
        $filename = 'export_crg_full_deduction_report_' . date('Y-m-d') . '.csv';

        // use keys as column titles
        $header = ['email', 'status', 'crg_status', 'country', 'language'];

        foreach ($data as $key => $datas) {
            $crg_id = $datas['crg_percentage_id'] ?: $datas['last_try_crg_percentage_id'];
            if ($datas['crg_deal'] == true || empty($crg_id)) {
                unset($data[$key]);
            }
