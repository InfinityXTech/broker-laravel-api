...
class MarketingCampaignTargetingLocation extends BaseModel
{
    use HasFactory, Notifiable;

    protected $connection = 'mongodb';
    protected $collection = 'marketing_campaign_targeting_locations';
    ...
    protected function regionName(): Attribute
    {
        return Attribute::make(
            get: function ($value, $attributes) {
                $result = [];
                $regions = GeneralHelper::regions($attributes['country_code'] ?? '', true);
                foreach($attributes['region_codes'] ?? [] as $region_code) {
                    $result[] = [
                        'code' => $region_code,
                        'name' => $regions[strtolower($region_code)]
                    ];
                }
                return $result;
            }
