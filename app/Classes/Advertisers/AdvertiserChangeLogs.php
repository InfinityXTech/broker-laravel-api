...
class AdvertiserChangeLogs
{
    ...
    private function _get_history_billing_general_balances($collection = '', $limit = 20)
    {
        $collections = [
            ...
            'marketing_advertiser_billing_entities',
            'marketing_advertiser_billing_chargebacks',
            'marketing_advertiser_billing_adjustments',
            'marketing_advertiser_billing_payment_methods',
            'marketing_advertiser_billing_payment_requests',
            'marketing_billings_log'
        ];

        $where = [
            '$or' => [
                ['main_foreign_key' => new ObjectId($this->advertiserId)],
                ['primary_key' => new ObjectId($this->advertiserId)],
            ],
            '$and' => [
                [
                    '$or' => [
        ...
        if (empty($collection)) {
            $where['$and'][] = [
                '$or' => [
                    ...
                    [
                        '$and' => [
                            ['collection' => 'marketing_advertiser_billing_payment_methods'],
        ...
        } elseif ($collection == 'marketing_advertiser_billing_payment_methods') {
    ...
    {
        $mongo = new MongoDBObjects('marketing_advertiser_billing_payment_methods', []);
    ...
    public function get_change_logs(bool $extended, string $collection, int $limit): array
    {
        ...
        $collections = [
            'marketing_advertisers' => 'General',
            'marketing_advertiser_billing_entities' => 'Billing Entities',
            'marketing_advertiser_billing_payment_methods' => 'Payment Methods',
            'marketing_advertiser_billing_payment_requests' => 'Payment Requests',
            'marketing_advertiser_billing_adjustments' => 'Adjustments',
            'marketing_advertiser_billing_chargebacks' => 'Chargebacks',
        ...
        foreach ($history_logs as $history) {
            ...
            switch ($history['collection']) {
                ...
                case 'marketing_advertiser_billing_entities': {
                ...
                case 'marketing_advertiser_billing_chargebacks': {
                        // if (isset($diff['type'])) {
                        //     $data .= 'Type: ' . $diff['type'];
                        // }
                ...
                case 'marketing_advertiser_billing_adjustments': {
                        if (isset($diff['amount'])) {
                ...
                case 'marketing_advertiser_billing_payment_methods': {
                        if (isset($diff['payment_method']) && isset($diff['status']) && (int)$diff['status'] == 1) {
                            $pm = $payment_methods[$diff['payment_method']];
                            $data = 'Changed Payment Method on ' . $pm['title'];
                        }
                        break;
                    }
                case 'marketing_advertiser_billing_payment_requests': {
